...
class SensorCluster(object):
        ...
        def __init__(self, ID, temp_addr, humidity_addr, humidity_chan, lux_addr, lux_mux_chan, adc_addr, mux_addr):
        ...
        def updateHumidity(self, bus):
            tries = 0
            STATUS = 0b11 << 30 # Create mask for STATUS (first two bits of 64 bit wide result)
            ...
            for i in range(3):
                sleep(.1) # wait 100ms to make sure the conversion takes place. 
                data = read_block_data(self.humidity_addr, 0)
                status = data&STATUS
                if status==0:
                    humidity = (data & HUMIDITY) >> 16
                    temp = data&TEMP >> 2
                    self.humidity = humidity/((2**14) - 2) # Normalize to relative humidity
                    # Temperature is ignored for now pending a successful block read.
                    return True
            print("Failed to update humidity. Check module connections"0
            return False                    
